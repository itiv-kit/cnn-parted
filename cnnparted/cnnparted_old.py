...
def main(args):
    ...
    if "neural-network" in config:
        workload = config['neural-network']
    else:
        workload = config['workload']

    node_components, link_components = conf_helper.get_system_components()
    accuracy_function = setup_workload(args.run_name, workload)
    step_runtime = [timer()]

    # Step 1 - Analysis
    ga = GraphAnalyzer(work_dir, args.run_name, tuple(workload['input-size']), args.p)
