...
class EyerissArchitectureMutator(ArchitectureMutator):
    ...
    def mutate_arch(self):
        ...
        eyeriss["local"][0]["attributes"]["memory_depth"] = self.config.glb_depth
        eyeriss["local"][1]["name"] = "DummyBuffer[0..{}]".format(self.config.meshX-1)

        eyeriss["subtree"][0]["name"] = "PE[0..{}]".format(self.config.num_pes-1)
        for component in eyeriss["subtree"][0]["local"]:
            component["attributes"]["meshX"] = self.config.meshX
        eyeriss["subtree"][0]["local"][0]["attributes"]["memory_depth"] = self.config.ifmap_spad_depth
        eyeriss["subtree"][0]["local"][1]["attributes"]["memory_depth"] = self.config.weights_spad_depth
        eyeriss["subtree"][0]["local"][2]["attributes"]["memory_depth"] = self.config.psum_spad_depth
